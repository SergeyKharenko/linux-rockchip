				 ...
				 size_t len, size_t offset)
{
	...
	regmap_bulk_write(lt8618->regmap, LT8618_REG_DDC_BASE, ddc_cfg,
			  ARRAY_SIZE(ddc_cfg));
				 ...
				 size_t len)
{
	...
	for (i = 0; i < batch_num; i++) {
		...
		ret = lt8618_ddc_fifo_fetch(lt8618, pfifo, block, len_max,
					    seek);
		if (ret) {
			DRM_ERROR(
...
static int lt8618_phase_config(struct lt8618 *lt8618)
{
	...
	if ((Jump_CNT == 2) || (Jump_CNT == 4) || (Jump_CNT == 6)) {
		regmap_write(lt8618->regmap, 0x8127,
			     (0x60 + (Jump_Num + (OK_CNT / 2)) % 0x0a));
	} else if (OK_CNT >= 0x09) {
...
static int lt8618_video_input_timing_check(struct lt8618 *lt8618)
{
	...
	regmap_bulk_read(lt8618->regmap, LT8618_REG_FREQ_METER2_BASE,
			 &freq_meter2, FREQ_METER2_LEN);
				...
				enum drm_bridge_attach_flags flags)
{
	...
	if (flags & DRM_BRIDGE_ATTACH_NO_CONNECTOR)
		return drm_bridge_attach(bridge->encoder, NULL, bridge, flags);
...
static int lt8618_video_output_pll_range(struct lt8618 *lt8618)
{
	...
	for (i = 0; i < ARRAY_SIZE(pll_range_cgf_regs); i++) {
		...
		pll_range_cgf[i].def = lt8618_pll_range_timing[0][lv][i];
	}
...
static void LT8618SXB_AVI_setting(struct lt8618 *lt8618)
{
	...
	if (VIC_Num == 95) {
		regmap_write(lt8618->regmap, 0x843d,
			     0x2a); //UD1 infoframe enable
				    ...
				    enum input_signal_sample_type signal_type)
{
	const struct reg_sequence lt8618_video_input_param_seq[] = {
		...
		{ LT8618_REG_INPUT_VIDEO_SYNC_GEN,
		  (video_type & 0x3) * 0x40 + 0x07 },
			...
			const struct i2c_device_id *id)
{
	...
	lt8618 = devm_kzalloc(&client->dev, sizeof(struct lt8618), GFP_KERNEL);
	if (!lt8618)
		return -ENOMEM;
	...
	lt8618->reset_gpio =
		devm_gpiod_get_optional(&client->dev, "rst", GPIOD_OUT_LOW);
